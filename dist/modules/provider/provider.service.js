...
const createProvider = async (data) => {
    return await prisma_1.prisma.provider.create({ data });
};
...
const getAllProviders = async (search, isFeatured) => {
...
const approveProvider = async (id, isApproved) => {
    return await prisma_1.prisma.provider.update({
        where: { id },
        data: { isApproved },
    });
...
const getProviderStats = async (userId) => {
    ...
    const pendingOrders = provider.orders.filter((o) => o.status === "placed").length;
    const avgRating = provider.meals.reduce((sum, m) => sum + (m.rating || 0), 0) / (totalMeals || 1);
    return {
        totalMeals,
        totalOrders,
        pendingOrders,
...
const getProviderMeals = async (userId) => {
    const provider = await prisma_1.prisma.provider.findUnique({
        where: { userId },
        include: {
            meals: {
